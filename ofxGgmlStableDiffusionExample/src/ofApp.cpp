...
std::string defaultControlFramesReadmeText() {
	return
		"Default control-frame folder for ofxGgmlStableDiffusion native video diffusion.\n"
		"\n"
...
void ofApp::setupHoloscanBridge() {
	...
	ofxGgmlStableDiffusionHoloscanSettings settings;
...
void ofApp::setupCreativeLoop() {
	...
	creativeLoop.setFrameCallback([this](const ofxGgmlStableDiffusionRealtimeVideoFrame& frame) {
...
//--------------------------------------------------------------
ofxGgmlStableDiffusionRealtimeVideoSettings ofApp::buildCreativeLoopSettings() const {
...
//--------------------------------------------------------------
ofxGgmlStableDiffusionRealtimeVideoRequest ofApp::buildCreativeLoopRequest() const {
...
void ofApp::drawCreativeLoopSection() {
	...
	ImGui::Text(
		...
		ofxGgmlStableDiffusionRealtimeVideoQualityLabel(creativeLoopLastQuality));
...
//--------------------------------------------------------------
ofxGgmlStableDiffusionContextSettings ofApp::buildContextSettings() const {
...
//--------------------------------------------------------------
ofxGgmlStableDiffusionVideoRequest ofApp::buildVideoRequest() const {
	ofxGgmlStableDiffusionVideoRequest request;
	const ofxGgmlStableDiffusionCapabilities capabilities = stableDiffusion.getCapabilities();
	request.initImage =
	...
	request.mode = ofxGgmlStableDiffusionVideoMode::Standard;
	request.loras = loras;
...
std::string ofApp::buildEquivalentSdCliCommand() const {
	...
	if (isImageToVideo) {
		...
		const ofxGgmlStableDiffusionVideoRequest request = buildVideoRequest();
...
//--------------------------------------------------------------
void ofApp::setup() {
	ofSetWindowTitle("ofxGgmlStableDiffusionExample");
	...
	imageModeEnum = ofxGgmlStableDiffusionImageMode::TextToImage;
	selectionMode = "KeepOrder";
	selectionModeEnum = ofxGgmlStableDiffusionImageSelectionMode::KeepOrder;
...
void ofApp::draw() {
	...
	ImGui::Begin("ofxGgmlStableDiffusion##foo0", NULL, flags);
	...
	ImGui::Begin("ofxGgmlStableDiffusion##foo1", NULL, flags);
	if (ImGui::TreeNodeEx("Image", ImGuiStyleVar_WindowPadding | ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (!isImageToVideo) {
			ofxGgmlStableDiffusionImageScore frameScore;
		...
		if (ImGui::Button("Save")) {
			if (isImageToVideo) {
				...
				if (stableDiffusion.copyVideoFramePixels(selectedImage, framePixels) && framePixels.isAllocated()) {
					ofSaveImage(
						framePixels,
						ofGetTimestampString("output/ofxGgmlStableDiffusion-%Y-%m-%d-%H-%M-%S.png"));
				}
			} else if (selectedImage >= 0 && selectedImage < static_cast<int>(textureVector.size())) {
				textureVector[selectedImage].readToPixels(pixels);
				ofSaveImage(pixels, ofGetTimestampString("output/ofxGgmlStableDiffusion-%Y-%m-%d-%H-%M-%S.png"));
			}
		}
		ImGui::TreePop();
	}
	...
	ImGui::Begin("ofxGgmlStableDiffusion##foo2", &logOpenSettings, flags);
	if (!logOpenSettings) {
	...
	if (ImGui::TreeNodeEx("Settings", ImGuiStyleVar_WindowPadding | ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (!isImageToVideo) {
			...
			if (ImGui::BeginCombo("Image Mode", imageMode, ImGuiComboFlags_NoArrowButton)) {
				for (int n = 0; n < IM_ARRAYSIZE(imageModeArray); n++) {
					const auto mode = static_cast<ofxGgmlStableDiffusionImageMode>(n);
					const bool supported =
						(mode == ofxGgmlStableDiffusionImageMode::TextToImage && capabilities.textToImage) ||
						(mode == ofxGgmlStableDiffusionImageMode::ImageToImage && capabilities.imageToImage) ||
						(mode == ofxGgmlStableDiffusionImageMode::Inpainting && capabilities.inpainting);
					if (!supported) {
						continue;
					}
		...
		if (!isImageToVideo) {
			...
			if (ImGui::BeginCombo("Selection", selectionMode, ImGuiComboFlags_NoArrowButton)) {
				for (int n = 0; n < IM_ARRAYSIZE(selectionModeArray); n++) {
					...
					if (ImGui::Selectable(selectionModeArray[n], is_selected)) {
						...
						selectionModeEnum = static_cast<ofxGgmlStableDiffusionImageSelectionMode>(n);
						stableDiffusion.setImageSelectionMode(selectionModeEnum);
						configureExampleRanker();
					}
					if (is_selected) {
			...
			if (selectionModeEnum != ofxGgmlStableDiffusionImageSelectionMode::KeepOrder) {
				ImGui::Dummy(ImVec2(0, 10));
		...
		ImGui::Text(
			"Model Family: %s",
			ofxGgmlStableDiffusionModelFamilyLabel(capabilities.modelFamily));
		ImGui::Dummy(ImVec2(0, 10));
		ImGui::TextWrapped(
			"%s",
		...
		if (imageModeEnum == ofxGgmlStableDiffusionImageMode::Inpainting) {
			ImGui::Dummy(ImVec2(0, 10));
		...
		if (isImageToVideo) {
			...
			const bool highNoiseOverridesAvailable =
				videoCapabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WAN ||
				videoCapabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WANI2V ||
				videoCapabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WANTI2V ||
				videoCapabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WANFLF2V ||
				videoCapabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WANVACE;
					...
					if (ImGui::BeginCombo("Interpolation", interpolationMode, ImGuiComboFlags_NoArrowButton)) {
						for (int n = 0; n < IM_ARRAYSIZE(interpolationModeArray); n++) {
							...
							if (ImGui::Selectable(interpolationModeArray[n], is_selected)) {
								...
								interpolationModeEnum = static_cast<ofxGgmlStableDiffusionInterpolationMode>(n);
							}
							if (is_selected) {
								ImGui::SetItemDefaultFocus();
							}
	...
	const bool needsMaskImage = imageModeEnum == ofxGgmlStableDiffusionImageMode::Inpainting;
	...
	if (ImGui::Button("Generate")) {
			...
			clampCurrentParametersToProfiles();
			if (isImageToVideo) {
				ofxGgmlStableDiffusionVideoRequest request = buildVideoRequest();
			...
			request.maskImage =
				(imageModeEnum == ofxGgmlStableDiffusionImageMode::Inpainting && useMaskGuide && maskGuideInput.data != nullptr) ?
					maskGuideInput :
					sd_image_t{0, 0, 0, nullptr};
			request.controlCond =
	...
	if (totalVideoFrames > 0) {
		...
		if (ImGui::Button("Save Video")) {
			const std::string path = ofToDataPath(
				ofGetTimestampString("output/ofxGgmlStableDiffusion-video-%Y-%m-%d-%H-%M-%S.avi"),
				true);
...
//--------------------------------------------------------------
void ofApp::applySelectedImageMode(ofxGgmlStableDiffusionImageMode mode) {
	imageModeEnum = mode;
	imageMode = imageModeArray[static_cast<int>(mode)];
	isTextToImage = (mode == ofxGgmlStableDiffusionImageMode::TextToImage);
...
void ofApp::applyRecommendedImageParameters() {
	imageParameterProfile = ofxGgmlStableDiffusionParameterTuningHelpers::resolveImageProfile(
...
void ofApp::applyRecommendedVideoParameters() {
	videoParameterProfile = ofxGgmlStableDiffusionParameterTuningHelpers::resolveVideoProfile(
		stableDiffusion.getContextSettings());
	width = 512;
	height = 512;
...
bool ofApp::usesInputImageMode() const {
	return ofxGgmlStableDiffusionImageModeUsesInputImage(imageModeEnum);
}

//--------------------------------------------------------------
void ofApp::syncAutomaticMediaMode(const ofxGgmlStableDiffusionCapabilities& capabilities) {
	...
	if (supportsVideoMode && !supportsImageMode) {
		if (!isImageToVideo) {
			isImageToVideo = true;
			videoUseInputImage =
				(capabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WANTI2V) ?
					false :
...
bool ofApp::videoModeSupportsTextAndImage() const {
	const ofxGgmlStableDiffusionCapabilities capabilities = stableDiffusion.getCapabilities();
	return capabilities.modelFamily == ofxGgmlStableDiffusionModelFamily::WANTI2V;
}
