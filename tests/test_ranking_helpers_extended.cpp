#include "core/ofxGgmlStableDiffusionRankingHelpers.h"
...
int main() {
	...
	// Test ofxGgmlStableDiffusionImageSelectionModeName - all modes
	ok &= expect(std::string(ofxGgmlStableDiffusionImageSelectionModeName(
		ofxGgmlStableDiffusionImageSelectionMode::KeepOrder)) == "KeepOrder",
		"keep-order label");
	ok &= expect(std::string(ofxGgmlStableDiffusionImageSelectionModeName(
		ofxGgmlStableDiffusionImageSelectionMode::Rerank)) == "Rerank",
		"rerank label");
	ok &= expect(std::string(ofxGgmlStableDiffusionImageSelectionModeName(
		ofxGgmlStableDiffusionImageSelectionMode::BestOnly)) == "BestOnly",
		"best-only label");

	// Test basic ranking
	{
		std::vector<ofxGgmlStableDiffusionImageScore> scores(4);
		scores[0].valid = true;
		...
		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(scores);
	...
	// Test empty scores
	{
		std::vector<ofxGgmlStableDiffusionImageScore> emptyScores;
		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(emptyScores);
		ok &= expect(order.empty(), "empty scores produces empty order");
	}

	// Test single score
	{
		std::vector<ofxGgmlStableDiffusionImageScore> singleScore(1);
		singleScore[0].valid = true;
		singleScore[0].score = 0.75f;

		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(singleScore);
	...
	{
		std::vector<ofxGgmlStableDiffusionImageScore> invalidScores(3);
		invalidScores[0].valid = false;
		invalidScores[1].valid = false;
		invalidScores[2].valid = false;

		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(invalidScores);
	...
	{
		std::vector<ofxGgmlStableDiffusionImageScore> identicalScores(4);
		...
		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(identicalScores);
	...
	{
		std::vector<ofxGgmlStableDiffusionImageScore> mixedScores(5);
		...
		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(mixedScores);
	...
	// Test extreme scores
	{
		std::vector<ofxGgmlStableDiffusionImageScore> extremeScores(3);
		...
		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(extremeScores);
	...
	{
		std::vector<ofxGgmlStableDiffusionImageScore> negativeScores(3);
		...
		const std::vector<std::size_t> order =
			ofxGgmlStableDiffusionBuildRankedImageOrder(negativeScores);
	...
	// Test ofxGgmlStableDiffusionImageScore metadata
	{
