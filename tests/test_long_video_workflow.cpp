...
int main() {
	...
	{
		ofxGgmlStableDiffusionLongVideoManifest manifest;
...

	{
		...
		manifest.preset = ofxGgmlStableDiffusionLongVideoPreset::Balanced;
		manifest.chunks = {
			{
				"chunk-1",
				"Opening",
		...
		const auto request =
			ofxGgmlStableDiffusionLongVideoWorkflow::buildChunkRequest(manifest, manifest.chunks.front());
		...
		const std::string chunkDir =
			ofxGgmlStableDiffusionLongVideoWorkflow::buildChunkOutputDirectory(manifest, manifest.chunks[1]);
		...
		const std::string manifestJson =
			ofxGgmlStableDiffusionLongVideoWorkflow::buildManifestJson(manifest);
		...
		std::vector<ofxGgmlStableDiffusionLongVideoChunkResult> chunkResults = {
		...
		const std::string playlistJson =
			ofxGgmlStableDiffusionLongVideoWorkflow::buildPlaylistManifestJson(manifest, chunkResults);
