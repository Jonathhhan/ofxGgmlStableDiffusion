...
struct ofxGgmlStableDiffusionPromptToken {
	std::string text;
	float weight = 1.0f;
};

/// Prompt analysis result
struct ofxGgmlStableDiffusionPromptAnalysis {
	int tokenCount = 0;
...
/// Prompt template
struct ofxGgmlStableDiffusionPromptTemplate {
	std::string name;
	std::string category;
...
class ofxGgmlStableDiffusionPromptHelpers {
public:
	static ofxGgmlStableDiffusionPromptHelpers& getInstance();
	...
	ofxGgmlStableDiffusionPromptAnalysis analyzePrompt(const std::string& prompt);
	...
	std::vector<ofxGgmlStableDiffusionPromptToken> parseEmphasis(const std::string& prompt);
	...
	ofxGgmlStableDiffusionPromptTemplate getTemplate(const std::string& name) const;

	/// Add a custom template
	/// @param templ Template to add
	void addTemplate(const ofxGgmlStableDiffusionPromptTemplate& templ);
...
private:
	ofxGgmlStableDiffusionPromptHelpers();
	~ofxGgmlStableDiffusionPromptHelpers() = default;

	// Prevent copying
	ofxGgmlStableDiffusionPromptHelpers(const ofxGgmlStableDiffusionPromptHelpers&) = delete;
	ofxGgmlStableDiffusionPromptHelpers& operator=(const ofxGgmlStableDiffusionPromptHelpers&) = delete;

	void initializeDefaultTemplates();
	void initializeDefaultNegativePresets();

	std::map<std::string, ofxGgmlStableDiffusionPromptTemplate> templates;
