...
enum class ofxGgmlStableDiffusionQuantization {
...
struct ofxGgmlStableDiffusionQuantizationLevelInfo {
...
struct ofxGgmlStableDiffusionQuantizationInfo {
	ofxGgmlStableDiffusionQuantization level = ofxGgmlStableDiffusionQuantization::None;
...
class ofxGgmlStableDiffusionQuantizationHelpers {
public:
	/// Get information about all available quantization levels
	/// @return Vector of quantization level information
	static std::vector<ofxGgmlStableDiffusionQuantizationLevelInfo> getAvailableLevels();

	/// Get information about a specific quantization level
	/// @param level Quantization level
	/// @return Information about the level
	static ofxGgmlStableDiffusionQuantizationLevelInfo getLevelInfo(ofxGgmlStableDiffusionQuantization level);
	...
	static ofxGgmlStableDiffusionQuantization getQuantizationByName(const std::string& name);

	/// Get name of a quantization level
	/// @param level Quantization level
	/// @return Name string
	static std::string getQuantizationName(ofxGgmlStableDiffusionQuantization level);
	...
	static size_t estimateQuantizedSize(size_t originalSizeMB, ofxGgmlStableDiffusionQuantization level);
	...
	static ofxGgmlStableDiffusionQuantization recommendQuantization(
	...
	static ofxGgmlStableDiffusionQuantization getPreset(const std::string& preset);

	/// Check if a quantization level is supported
	/// @param level Quantization level to check
	/// @return True if supported
	static bool isSupported(ofxGgmlStableDiffusionQuantization level);

	/// Get memory reduction percentage for a quantization level
	/// @param level Quantization level
	/// @return Memory reduction percentage (0-100)
	static float getMemoryReduction(ofxGgmlStableDiffusionQuantization level);

	/// Get speed multiplier for a quantization level
	/// @param level Quantization level
	/// @return Speed multiplier (1.0 = same as F32, >1.0 = faster)
	static float getSpeedMultiplier(ofxGgmlStableDiffusionQuantization level);

private:
	static std::vector<ofxGgmlStableDiffusionQuantizationLevelInfo> levelInfoCache;
