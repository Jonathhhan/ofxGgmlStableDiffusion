...
ofxGgmlStableDiffusionRealtimeSession::ofxGgmlStableDiffusionRealtimeSession() {
}
...
bool ofxGgmlStableDiffusionRealtimeSession::start(const ofxGgmlStableDiffusionRealtimeSettings& settings_) {
	if (active) {
		ofLogWarning("ofxGgmlStableDiffusionRealtimeSession") << "Session already active";
		return false;
	}
	...
	stats = ofxGgmlStableDiffusionRealtimeStats();
	...
	ofLogNotice("ofxGgmlStableDiffusionRealtimeSession")
...
void ofxGgmlStableDiffusionRealtimeSession::setGenerator(ofxGgmlStableDiffusion* sd) {
	generator = sd;
}

void ofxGgmlStableDiffusionRealtimeSession::stop() {
	if (!active) {
		return;
	}
...
bool ofxGgmlStableDiffusionRealtimeSession::isActive() const {
	return active;
}

bool ofxGgmlStableDiffusionRealtimeSession::submit(const ofxGgmlStableDiffusionRealtimeRequest& request) {
	if (!active) {
		ofLogWarning("ofxGgmlStableDiffusionRealtimeSession") << "Session not active";
		return false;
	}

	if (!generator) {
		ofLogWarning("ofxGgmlStableDiffusionRealtimeSession")
	...
	const bool mustDropIfBusy =
		...
		settings.mode == ofxGgmlStableDiffusionRealtimeMode::LowLatency);
...
void ofxGgmlStableDiffusionRealtimeSession::update() {
	if (!active || !generator) {
		return;
	}
	...
	if (warmingUp) {
		...
		if (resultCallback && result.success) {
			try {
				...
				ofLogWarning("ofxGgmlStableDiffusionRealtimeSession") << "Result callback threw: " << e.what();
			} catch (...) {
				ofLogWarning("ofxGgmlStableDiffusionRealtimeSession") << "Result callback threw an unknown exception";
			}
		}
	}
...
void ofxGgmlStableDiffusionRealtimeSession::updatePrompt(const std::string& prompt) {
	std::lock_guard<std::mutex> lock(requestMutex);
	pendingRequest.prompt = prompt;
}

void ofxGgmlStableDiffusionRealtimeSession::updateCfgScale(float cfgScale) {
	std::lock_guard<std::mutex> lock(requestMutex);
	pendingRequest.cfgScale = cfgScale;
}

void ofxGgmlStableDiffusionRealtimeSession::updateStrength(float strength) {
	std::lock_guard<std::mutex> lock(requestMutex);
	pendingRequest.strength = strength;
}

void ofxGgmlStableDiffusionRealtimeSession::updateNegativePrompt(const std::string& negativePrompt) {
	std::lock_guard<std::mutex> lock(requestMutex);
	pendingRequest.negativePrompt = negativePrompt;
}

void ofxGgmlStableDiffusionRealtimeSession::updateSeed(int seed) {
	std::lock_guard<std::mutex> lock(requestMutex);
	pendingRequest.seed = seed;
}

void ofxGgmlStableDiffusionRealtimeSession::updateSampleSteps(int steps) {
	std::lock_guard<std::mutex> lock(requestMutex);
...
void ofxGgmlStableDiffusionRealtimeSession::updateDimensions(int width, int height) {
	std::lock_guard<std::mutex> lock(requestMutex);
	pendingRequest.width = width;
	pendingRequest.height = height;
}

void ofxGgmlStableDiffusionRealtimeSession::setResultCallback(ofxSdRealtimeResultCallback callback) {
	resultCallback = callback;
}

void ofxGgmlStableDiffusionRealtimeSession::setLatencyCallback(ofxSdRealtimeLatencyCallback callback) {
	latencyCallback = callback;
}

ofxGgmlStableDiffusionRealtimeStats ofxGgmlStableDiffusionRealtimeSession::getStats() const {
	return stats;
}

ofxGgmlStableDiffusionRealtimeSettings ofxGgmlStableDiffusionRealtimeSession::getSettings() const {
	return settings;
}

void ofxGgmlStableDiffusionRealtimeSession::resetStats() {
	stats = ofxGgmlStableDiffusionRealtimeStats();
	stats.sessionStartTime = ofGetElapsedTimeMillis();
...
ofxGgmlStableDiffusionResult ofxGgmlStableDiffusionRealtimeSession::getLastResult() const {
	std::lock_guard<std::mutex> lock(requestMutex);
	return lastResult;
}

bool ofxGgmlStableDiffusionRealtimeSession::isGenerating() const {
	return generationInFlight;
}

int ofxGgmlStableDiffusionRealtimeSession::getCurrentSampleSteps() const {
	return currentSampleSteps;
}

bool ofxGgmlStableDiffusionRealtimeSession::warmup() {
	if (!active) {
		ofLogWarning("ofxGgmlStableDiffusionRealtimeSession") << "warmup: session not active";
		return false;
	}
	...
	ofxGgmlStableDiffusionImageRequest warmupReq;
	...
	warmupReq.mode = ofxGgmlStableDiffusionImageMode::TextToImage;
	...
	ofLogNotice("ofxGgmlStableDiffusionRealtimeSession") << "Warming up...";
	return true;
}

int ofxGgmlStableDiffusionRealtimeSession::clampToStepRange(int steps) const {
	return std::max(settings.minSampleSteps, std::min(steps, settings.maxSampleSteps));
}

void ofxGgmlStableDiffusionRealtimeSession::processQueue() {
	if (!generator || generator->isGenerating() || generationInFlight) {
		return;
	}

	ofxGgmlStableDiffusionRealtimeRequest req;
	{
		std::lock_guard<std::mutex> lock(requestMutex);
	...
	ofxGgmlStableDiffusionImageRequest imageReq;
	...
	imageReq.mode = ofxGgmlStableDiffusionImageMode::TextToImage;
...
void ofxGgmlStableDiffusionRealtimeSession::updateStats(float latencyMs) {
