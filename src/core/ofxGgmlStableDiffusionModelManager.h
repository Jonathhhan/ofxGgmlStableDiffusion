...
struct ofxGgmlStableDiffusionModelInfo {
...
/// Model cache entry
struct ofxGgmlStableDiffusionModelCacheEntry {
...
class ofxGgmlStableDiffusionModelManager {
public:
	...
	std::vector<ofxGgmlStableDiffusionModelInfo> scanModelsInDirectory(const std::string& directory);

	/// Extract metadata from a model file
	ofxGgmlStableDiffusionModelInfo extractModelInfo(const std::string& modelPath);

	/// Validate a model file
	bool validateModel(const std::string& modelPath, std::string& errorMessage);

	/// Preload a model into cache
	bool preloadModel(const ofxGgmlStableDiffusionModelInfo& modelInfo, std::string& errorMessage);
	...
	std::optional<ofxGgmlStableDiffusionModelInfo> getCachedModelInfo(const std::string& modelPath) const;
...
private:
	std::map<std::string, ofxGgmlStableDiffusionModelCacheEntry> modelCache;
	...
	sd_ctx_t* loadModelContext(const ofxGgmlStableDiffusionModelInfo& modelInfo, std::string& errorMessage);
