#include "ofxGgmlStableDiffusionPromptHelpers.h"
#include "ofxGgmlStableDiffusionStringUtils.h"
#include <algorithm>
#include <regex>
#include <sstream>

ofxGgmlStableDiffusionPromptHelpers& ofxGgmlStableDiffusionPromptHelpers::getInstance() {
	static ofxGgmlStableDiffusionPromptHelpers instance;
	return instance;
}

ofxGgmlStableDiffusionPromptHelpers::ofxGgmlStableDiffusionPromptHelpers() {
	initializeDefaultTemplates();
	initializeDefaultNegativePresets();
}

std::string ofxGgmlStableDiffusionPromptHelpers::applyTemplate(
	const std::string& templateName,
	const std::map<std::string, std::string>& variables) {

	auto it = templates.find(templateName);
	if (it == templates.end()) {
		ofLogWarning("ofxGgmlStableDiffusionPromptHelpers")
...
ofxGgmlStableDiffusionPromptAnalysis ofxGgmlStableDiffusionPromptHelpers::analyzePrompt(
	const std::string& prompt) {

	ofxGgmlStableDiffusionPromptAnalysis analysis;
...
std::string ofxGgmlStableDiffusionPromptHelpers::getNegativePreset(const std::string& presetName) {
...
std::vector<ofxGgmlStableDiffusionPromptToken> ofxGgmlStableDiffusionPromptHelpers::parseEmphasis(
	const std::string& prompt) {

	std::vector<ofxGgmlStableDiffusionPromptToken> tokens;
	...
	while (std::regex_search(searchStart, prompt.cend(), match, emphasisRegex)) {
		...
		ofxGgmlStableDiffusionPromptToken token;
...
int ofxGgmlStableDiffusionPromptHelpers::loadTemplates(const std::string& directory) {
	int loaded = 0;
	...
	for (size_t i = 0; i < dir.size(); i++) {
		try {
			...
			ofxGgmlStableDiffusionPromptTemplate templ;
	...
	ofLogNotice("ofxGgmlStableDiffusionPromptHelpers")
		<< "Loaded " << loaded << " templates from " << directory;

	return loaded;
}

std::vector<std::string> ofxGgmlStableDiffusionPromptHelpers::getTemplateCategories() const {
...
std::vector<std::string> ofxGgmlStableDiffusionPromptHelpers::getTemplatesInCategory(
...
ofxGgmlStableDiffusionPromptTemplate ofxGgmlStableDiffusionPromptHelpers::getTemplate(
	...
	return ofxGgmlStableDiffusionPromptTemplate();
}

void ofxGgmlStableDiffusionPromptHelpers::addTemplate(const ofxGgmlStableDiffusionPromptTemplate& templ) {
	if (!templ.name.empty()) {
		templates[templ.name] = templ;
	}
}

std::map<std::string, std::string> ofxGgmlStableDiffusionPromptHelpers::getAllNegativePresets() const {
	return negativePresets;
}

int ofxGgmlStableDiffusionPromptHelpers::estimateTokenCount(const std::string& text) const {
...
std::string ofxGgmlStableDiffusionPromptHelpers::cleanPrompt(const std::string& prompt) const {
...
void ofxGgmlStableDiffusionPromptHelpers::initializeDefaultTemplates() {
...
void ofxGgmlStableDiffusionPromptHelpers::initializeDefaultNegativePresets() {
