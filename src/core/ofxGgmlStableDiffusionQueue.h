...
enum class ofxGgmlStableDiffusionPriority {
	Low = 0,
	Normal = 1,
	High = 2,
	Critical = 3
...
/// Queue request state
enum class ofxGgmlStableDiffusionQueueState {
	Queued = 0,
	Processing,
	Completed,
	Failed,
...
/// Queue request entry
struct ofxGgmlStableDiffusionQueueRequest {
	int requestId = -1;
	ofxGgmlStableDiffusionPriority priority = ofxGgmlStableDiffusionPriority::Normal;
	ofxGgmlStableDiffusionQueueState state = ofxGgmlStableDiffusionQueueState::Queued;
	ofxGgmlStableDiffusionTask taskType = ofxGgmlStableDiffusionTask::None;

	// Request data (one will be set based on taskType)
	ofxGgmlStableDiffusionImageRequest imageRequest;
	ofxGgmlStableDiffusionVideoRequest videoRequest;
	...
	bool isImageGeneration() const {
		return taskType == ofxGgmlStableDiffusionTask::TextToImage ||
			   taskType == ofxGgmlStableDiffusionTask::ImageToImage ||
			   taskType == ofxGgmlStableDiffusionTask::Inpainting;
	}

	bool isVideoGeneration() const {
		return taskType == ofxGgmlStableDiffusionTask::ImageToVideo;
	}

	bool isModelLoad() const {
		return taskType == ofxGgmlStableDiffusionTask::LoadModel;
	}
...
struct ofxGgmlStableDiffusionRequestComparator {
	bool operator()(const std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>& a,
					const std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>& b) const {
...
class ofxGgmlStableDiffusionQueue {
public:
	ofxGgmlStableDiffusionQueue();
	~ofxGgmlStableDiffusionQueue();

	/// Add an image generation request to the queue
	int addImageRequest(const ofxGgmlStableDiffusionImageRequest& request,
						ofxGgmlStableDiffusionPriority priority = ofxGgmlStableDiffusionPriority::Normal,
						const std::string& tag = "");

	/// Add a video generation request to the queue
	int addVideoRequest(const ofxGgmlStableDiffusionVideoRequest& request,
						ofxGgmlStableDiffusionPriority priority = ofxGgmlStableDiffusionPriority::Normal,
						const std::string& tag = "");

	/// Add a model load request to the queue
	int addModelLoadRequest(const ofxGgmlStableDiffusionContextSettings& settings,
							ofxGgmlStableDiffusionPriority priority = ofxGgmlStableDiffusionPriority::High,
							const std::string& tag = "");

	/// Set callback for request completion
	void setCompletionCallback(int requestId, std::function<void(const ofxGgmlStableDiffusionResult&)> callback);
	...
	std::shared_ptr<ofxGgmlStableDiffusionQueueRequest> getNextRequest();

	/// Mark a request as processing
	void markRequestProcessing(int requestId);

	/// Mark a request as completed
	void markRequestCompleted(int requestId, const ofxGgmlStableDiffusionResult& result);

	/// Mark a request as failed
	void markRequestFailed(int requestId, const std::string& errorMessage);

	/// Get request by ID
	std::shared_ptr<ofxGgmlStableDiffusionQueueRequest> getRequest(int requestId);

	/// Get all requests with a specific state
	std::vector<std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>> getRequestsByState(ofxGgmlStableDiffusionQueueState state) const;

	/// Get all requests with a specific tag
	std::vector<std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>> getRequestsByTag(const std::string& tag) const;
	...
	std::shared_ptr<ofxGgmlStableDiffusionQueueRequest> getCurrentRequest() const;
...
private:
	...
	std::priority_queue<std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>,
						std::vector<std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>>,
						ofxGgmlStableDiffusionRequestComparator> requestQueue;

	std::map<int, std::shared_ptr<ofxGgmlStableDiffusionQueueRequest>> allRequests;
	std::shared_ptr<ofxGgmlStableDiffusionQueueRequest> currentRequest;
	...
	std::shared_ptr<ofxGgmlStableDiffusionQueueRequest> createRequest(ofxGgmlStableDiffusionTask taskType,
