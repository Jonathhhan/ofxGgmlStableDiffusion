#include "ofxGgmlStableDiffusionBatchProcessor.h"
...
const ofxGgmlStableDiffusionImageFrame* pickRepresentativeImage(
...
ofxGgmlStableDiffusionBatchProcessor::ofxGgmlStableDiffusionBatchProcessor() {
	qualityScoreFunc = [](const ofxGgmlStableDiffusionResult& result) {
...
void ofxGgmlStableDiffusionBatchProcessor::setGenerator(ofxGgmlStableDiffusion* sd) {
	generator = sd;
}

ofxGgmlStableDiffusion* ofxGgmlStableDiffusionBatchProcessor::getGenerator() const {
	return generator;
}

bool ofxGgmlStableDiffusionBatchProcessor::hasGenerator() const {
	return generator != nullptr;
}

ofxGgmlStableDiffusionBatchResult ofxGgmlStableDiffusionBatchProcessor::generateGrid(
	const ofxGgmlStableDiffusionGridSettings& settings) {
	...
	std::vector<std::vector<ofxGgmlStableDiffusionResult>> gridResults(
		settings.yValues.size(),
		std::vector<ofxGgmlStableDiffusionResult>(settings.xValues.size()));
	int entryIndex = 0;
	...
	for (std::size_t y = 0; y < settings.yValues.size() && !cancelRequested; ++y) {
		for (std::size_t x = 0; x < settings.xValues.size() && !cancelRequested; ++x) {
			...
			ofxGgmlStableDiffusionImageRequest request = settings.baseRequest;
...
ofxGgmlStableDiffusionSweepResult ofxGgmlStableDiffusionBatchProcessor::parameterSweep(
	const ofxGgmlStableDiffusionSweepSettings& settings) {
	ofxGgmlStableDiffusionSweepResult sweepResult;
...
ofxGgmlStableDiffusionComparisonResult ofxGgmlStableDiffusionBatchProcessor::compareAB(
	const ofxGgmlStableDiffusionImageRequest& requestA,
	const ofxGgmlStableDiffusionImageRequest& requestB) {
	ofxGgmlStableDiffusionComparisonResult comparison;
	comparison.nameA = "A";
	comparison.nameB = "B";
	if (!generator) {
...
ofxGgmlStableDiffusionBatchResult ofxGgmlStableDiffusionBatchProcessor::processBatch(
...
void ofxGgmlStableDiffusionBatchProcessor::setProgressCallback(
	std::function<void(int, int, const std::string&)> callback) {
	progressCallback = callback;
}

void ofxGgmlStableDiffusionBatchProcessor::setQualityScoringFunction(
	std::function<float(const ofxGgmlStableDiffusionResult&)> scoreFunc) {
	qualityScoreFunc = scoreFunc ? scoreFunc : [](const ofxGgmlStableDiffusionResult& result) {
		return result.success ? 1.0f : 0.0f;
	};
}

void ofxGgmlStableDiffusionBatchProcessor::cancel() {
...
bool ofxGgmlStableDiffusionBatchProcessor::isRunning() const {
	return running;
}

void ofxGgmlStableDiffusionBatchProcessor::setPollIntervalMs(int intervalMs) {
	pollIntervalMs = std::max(1, intervalMs);
}

int ofxGgmlStableDiffusionBatchProcessor::getPollIntervalMs() const {
	return pollIntervalMs;
}

void ofxGgmlStableDiffusionBatchProcessor::setExecutionTimeoutMs(int timeoutMs) {
	executionTimeoutMs = std::max(1, timeoutMs);
}

int ofxGgmlStableDiffusionBatchProcessor::getExecutionTimeoutMs() const {
	return executionTimeoutMs;
}

bool ofxGgmlStableDiffusionBatchProcessor::runImageRequest(
	const ofxGgmlStableDiffusionImageRequest& request,
	ofxGgmlStableDiffusionResult& result,
	std::string& errorMessage) {
	if (!generator) {
		errorMessage = "No generator attached";
		result = ofxGgmlStableDiffusionResult();
		result.success = false;
		result.error = errorMessage;
		return false;
	}
...
bool ofxGgmlStableDiffusionBatchProcessor::waitForCurrentGeneration(
	ofxGgmlStableDiffusionResult& result,
	std::string& errorMessage) {
	if (!generator) {
		errorMessage = "No generator attached";
...
float ofxGgmlStableDiffusionBatchProcessor::scoreResult(
	const ofxGgmlStableDiffusionResult& result) const {
	return qualityScoreFunc ? qualityScoreFunc(result) : 0.0f;
}

void ofxGgmlStableDiffusionBatchProcessor::recordBatchEntry(
	ofxGgmlStableDiffusionBatchResult& batchResult,
	const ofxGgmlStableDiffusionResult& result,
	int entryIndex,
...
bool ofxGgmlStableDiffusionBatchProcessor::exportResultImage(
	const ofxGgmlStableDiffusionResult& result,
...
void ofxGgmlStableDiffusionBatchProcessor::applyParameterValue(
	ofxGgmlStableDiffusionImageRequest& request,
	ofxGgmlStableDiffusionParameter param,
	float value) {
	switch (param) {
		case ofxGgmlStableDiffusionParameter::CfgScale:
			request.cfgScale = value;
			break;
		case ofxGgmlStableDiffusionParameter::SampleSteps:
			request.sampleSteps = static_cast<int>(value);
			break;
		case ofxGgmlStableDiffusionParameter::Strength:
			request.strength = value;
			break;
		case ofxGgmlStableDiffusionParameter::Seed: {
		...
		case ofxGgmlStableDiffusionParameter::Width:
			request.width = static_cast<int>(value);
			break;
		case ofxGgmlStableDiffusionParameter::Height:
			request.height = static_cast<int>(value);
			break;
		case ofxGgmlStableDiffusionParameter::SamplerMethod:
			request.sampleMethod = static_cast<sample_method_t>(static_cast<int>(value));
			break;
		case ofxGgmlStableDiffusionParameter::Schedule:
			request.schedule = static_cast<scheduler_t>(static_cast<int>(value));
			break;
		case ofxGgmlStableDiffusionParameter::BatchCount:
			request.batchCount = static_cast<int>(value);
			break;
	}
}

float ofxGgmlStableDiffusionBatchProcessor::getParameterValue(
	const ofxGgmlStableDiffusionImageRequest& request,
...
std::string ofxGgmlStableDiffusionBatchProcessor::getParameterName(
	ofxGgmlStableDiffusionParameter param) const {
	switch (param) {
		case ofxGgmlStableDiffusionParameter::CfgScale: return "CFG Scale";
		case ofxGgmlStableDiffusionParameter::SampleSteps: return "Sample Steps";
		case ofxGgmlStableDiffusionParameter::Strength: return "Strength";
		case ofxGgmlStableDiffusionParameter::Seed: return "Seed";
		case ofxGgmlStableDiffusionParameter::Width: return "Width";
		case ofxGgmlStableDiffusionParameter::Height: return "Height";
		case ofxGgmlStableDiffusionParameter::SamplerMethod: return "Sampler Method";
		case ofxGgmlStableDiffusionParameter::Schedule: return "Schedule";
		case ofxGgmlStableDiffusionParameter::BatchCount: return "Batch Count";
		default: return "Unknown";
	}
}

std::vector<float> ofxGgmlStableDiffusionBatchProcessor::generateStepValues(
	float minVal,
	float maxVal,
	int steps,
	...
	switch (mode) {
		case ofxGgmlStableDiffusionStepMode::Linear: {
		...
		case ofxGgmlStableDiffusionStepMode::Logarithmic: {
			if (minVal <= 0.0f) {
				ofLogWarning("ofxGgmlStableDiffusionBatchProcessor")
...
bool ofxGgmlStableDiffusionBatchResult::exportMetadata(const std::string& path) const {
	ofJson json;
...
bool ofxGgmlStableDiffusionComparisonResult::exportComparison(const std::string& path) const {
...
ofImage ofxGgmlStableDiffusionBatchProcessor::createGridImage(
