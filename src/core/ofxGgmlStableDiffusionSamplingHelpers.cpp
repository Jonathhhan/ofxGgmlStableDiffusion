#include "ofxGgmlStableDiffusionSamplingHelpers.h"
#include <algorithm>
#include <cctype>

// Sampling Presets
ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::Quality() {
	return {"Quality", "High quality output with detailed results",
		DPMPP2M_SAMPLE_METHOD, KARRAS_SCHEDULER, 30, 7.5f};
}

ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::UltraQuality() {
	return {"Ultra Quality", "Maximum quality for final renders",
		DPMPP2M_SAMPLE_METHOD, KARRAS_SCHEDULER, 50, 8.0f};
}

ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::Fast() {
	return {"Fast", "Quick preview with acceptable quality",
		EULER_A_SAMPLE_METHOD, DISCRETE_SCHEDULER, 15, 7.0f};
}

ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::UltraFast() {
	return {"Ultra Fast", "Fastest generation for rapid iteration",
		EULER_A_SAMPLE_METHOD, DISCRETE_SCHEDULER, 8, 6.0f};
}

ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::Balanced() {
	return {"Balanced", "Good balance of quality and speed",
		EULER_A_SAMPLE_METHOD, KARRAS_SCHEDULER, 20, 7.0f};
}

ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::LCM() {
	return {"LCM", "Latent Consistency Model (4-8 steps)",
		LCM_SAMPLE_METHOD, LCM_SCHEDULER, 4, 1.0f};
}

ofxGgmlStableDiffusionSamplingPreset ofxGgmlStableDiffusionSamplingPreset::TCD() {
	return {"TCD", "Trajectory Consistency Distillation",
		TCD_SAMPLE_METHOD, SGM_UNIFORM_SCHEDULER, 8, 3.0f};
}

// Sampler Information
std::vector<ofxGgmlStableDiffusionSamplerInfo> ofxGgmlStableDiffusionSamplingHelpers::getAllSamplers() {
	return {
...
std::vector<ofxGgmlStableDiffusionSchedulerInfo> ofxGgmlStableDiffusionSamplingHelpers::getAllSchedulers() {
	return {
...
ofxGgmlStableDiffusionSamplerInfo ofxGgmlStableDiffusionSamplingHelpers::getSamplerInfo(sample_method_t method) {
...
ofxGgmlStableDiffusionSchedulerInfo ofxGgmlStableDiffusionSamplingHelpers::getSchedulerInfo(scheduler_t scheduler) {
...
sample_method_t ofxGgmlStableDiffusionSamplingHelpers::getSamplerByName(const std::string& name) {
...
scheduler_t ofxGgmlStableDiffusionSamplingHelpers::getSchedulerByName(const std::string& name) {
	std::string lowerName = toLower(name);
	auto schedulers = getAllSchedulers();
	for (const auto& info : schedulers) {
		if (toLower(info.name) == lowerName) {
...
std::string ofxGgmlStableDiffusionSamplingHelpers::getSamplerName(sample_method_t method) {
	return getSamplerInfo(method).name;
}

std::string ofxGgmlStableDiffusionSamplingHelpers::getSchedulerName(scheduler_t scheduler) {
	return getSchedulerInfo(scheduler).name;
}

std::vector<std::pair<sample_method_t, scheduler_t>>
ofxGgmlStableDiffusionSamplingHelpers::getRecommendedCombinations() {
	return {
...
bool ofxGgmlStableDiffusionSamplingHelpers::isValidStepCount(sample_method_t method, int steps) {
	auto info = getSamplerInfo(method);
	return steps >= info.recommendedMinSteps && steps <= info.recommendedMaxSteps;
}

int ofxGgmlStableDiffusionSamplingHelpers::getRecommendedSteps(sample_method_t method, float qualityLevel) {
	auto info = getSamplerInfo(method);
...
std::vector<ofxGgmlStableDiffusionSamplingPreset> ofxGgmlStableDiffusionSamplingHelpers::getAllPresets() {
	return {
		ofxGgmlStableDiffusionSamplingPreset::UltraQuality(),
		ofxGgmlStableDiffusionSamplingPreset::Quality(),
		ofxGgmlStableDiffusionSamplingPreset::Balanced(),
		ofxGgmlStableDiffusionSamplingPreset::Fast(),
		ofxGgmlStableDiffusionSamplingPreset::UltraFast(),
		ofxGgmlStableDiffusionSamplingPreset::LCM(),
		ofxGgmlStableDiffusionSamplingPreset::TCD()
	};
}
