...
bool copyPixelsToPreview(
	const ofxGgmlStableDiffusionImageFrame& image,
	ofTexture* texture,
	ofxGgmlStableDiffusionHoloscanPreviewFrame* preview) {
...
ofxGgmlStableDiffusionImageRequest makeBridgeRequest(
	const ofxGgmlStableDiffusionHoloscanConditioningPacket& packet) {
...
void clearPreview(ofTexture* texture, ofxGgmlStableDiffusionHoloscanPreviewFrame* preview) {
	if (texture) {
		texture->clear();
	}
	if (preview) {
...
struct HoloscanSharedRuntimeState {
	...
	std::deque<ofxGgmlStableDiffusionHoloscanFramePacket> pendingFrames;
	std::vector<ofxGgmlStableDiffusionImageFrame> finishedImages;
	ofxGgmlStableDiffusionHoloscanPreviewFrame previewFrame;
...
public:
	...
	void setup(holoscan::OperatorSpec& spec) override {
		spec.output<std::shared_ptr<ofxGgmlStableDiffusionHoloscanFramePacket>>("out");
	...
	void compute(
		...
		ofxGgmlStableDiffusionHoloscanFramePacket packet;
		{
		...
		op_output.emit(
			std::make_shared<ofxGgmlStableDiffusionHoloscanFramePacket>(std::move(packet)),
			"out");
	}
...
public:
	...
	void setup(holoscan::OperatorSpec& spec) override {
		spec.input<std::shared_ptr<ofxGgmlStableDiffusionHoloscanFramePacket>>("in");
		spec.output<std::shared_ptr<ofxGgmlStableDiffusionHoloscanConditioningPacket>>("out");
		spec.param(allocator_,
			"allocator",
			"Allocator",
	...
	void compute(
		...
		holoscan::ExecutionContext&) override {
		auto packet =
			op_input.receive<std::shared_ptr<ofxGgmlStableDiffusionHoloscanFramePacket>>("in");
		if (!packet) {
			return;
		}
		...
		ofxGgmlStableDiffusionHoloscanConditioningPacket conditioning;
		...
		op_output.emit(
			std::make_shared<ofxGgmlStableDiffusionHoloscanConditioningPacket>(
...
public:
	...
	ofxGgmlStableDiffusion* diffusion = nullptr;
	std::shared_ptr<HoloscanSharedRuntimeState> sharedState;

	void setup(holoscan::OperatorSpec& spec) override {
		spec.input<std::shared_ptr<ofxGgmlStableDiffusionHoloscanConditioningPacket>>("in");
		spec.output<std::shared_ptr<ofxGgmlStableDiffusionHoloscanImagePacket>>("out");
		spec.param(allocator_,
			"allocator",
			"Allocator",
	...
	void compute(
		...
		holoscan::ExecutionContext&) override {
		auto packet =
			op_input.receive<std::shared_ptr<ofxGgmlStableDiffusionHoloscanConditioningPacket>>(
				"in");
		if (!packet) {
			return;
		}
		...
		try {
			...
			ofxGgmlStableDiffusionHoloscanImagePacket imagePacket;
			...
			op_output.emit(
				std::make_shared<ofxGgmlStableDiffusionHoloscanImagePacket>(
...
public:
	...
	void setup(holoscan::OperatorSpec& spec) override {
		spec.input<std::shared_ptr<ofxGgmlStableDiffusionHoloscanImagePacket>>("in");
		spec.param(allocator_,
			"allocator",
			"Allocator",
	...
	void compute(
		...
		holoscan::ExecutionContext&) override {
		auto packet =
			op_input.receive<std::shared_ptr<ofxGgmlStableDiffusionHoloscanImagePacket>>("in");
		if (!packet) {
			return;
		}
...
private:
	ofxGgmlStableDiffusion* diffusion_ = nullptr;
	std::shared_ptr<HoloscanSharedRuntimeState> sharedState_;
	ofxGgmlStableDiffusionHoloscanSettings settings_;
};
...
struct ofxGgmlStableDiffusionHoloscanBridge::Impl {
	ofxGgmlStableDiffusion* diffusion = nullptr;
	ofxGgmlStableDiffusionHoloscanSettings settings;
...
ofxGgmlStableDiffusionHoloscanBridge::ofxGgmlStableDiffusionHoloscanBridge()
...
ofxGgmlStableDiffusionHoloscanBridge::~ofxGgmlStableDiffusionHoloscanBridge() = default;

bool ofxGgmlStableDiffusionHoloscanBridge::setup(
	ofxGgmlStableDiffusion* diffusion,
	...
	if (!impl_->configured) {
		impl_->lastError = "Holoscan bridge setup requires a valid ofxGgmlStableDiffusion instance.";
	}
#if OFXSD_HAS_HOLOSCAN
...
void ofxGgmlStableDiffusionHoloscanBridge::shutdown() {
	stop();
...
bool ofxGgmlStableDiffusionHoloscanBridge::startImagePipeline() {
...
void ofxGgmlStableDiffusionHoloscanBridge::stop() {
...
void ofxGgmlStableDiffusionHoloscanBridge::update() {
	...
	ofxGgmlStableDiffusionHoloscanFramePacket frame;
	{
		std::lock_guard<std::mutex> lock(impl_->mutex);
...
void ofxGgmlStableDiffusionHoloscanBridge::submitFrame(
...
void ofxGgmlStableDiffusionHoloscanBridge::submitPrompt(
...
bool ofxGgmlStableDiffusionHoloscanBridge::hasPreviewFrame() const {
	return impl_->previewFrame.valid && impl_->previewTexture.isAllocated();
}

const ofTexture& ofxGgmlStableDiffusionHoloscanBridge::getPreviewTexture() const {
	return impl_->previewTexture;
}

ofxGgmlStableDiffusionHoloscanPreviewFrame
ofxGgmlStableDiffusionHoloscanBridge::getPreviewFrameCopy() const {
	return impl_->previewFrame;
}

std::vector<ofxGgmlStableDiffusionImageFrame>
ofxGgmlStableDiffusionHoloscanBridge::consumeFinishedImages() {
#if OFXSD_HAS_HOLOSCAN
	if (impl_->usingHoloscanRuntime && impl_->holoscanState) {
		std::lock_guard<std::mutex> lock(impl_->holoscanState->mutex);
...
bool ofxGgmlStableDiffusionHoloscanBridge::isConfigured() const {
	return impl_->configured;
}

bool ofxGgmlStableDiffusionHoloscanBridge::isRunning() const {
	return impl_->running;
}

bool ofxGgmlStableDiffusionHoloscanBridge::isHoloscanAvailable() const {
	return impl_->holoscanAvailable;
}

std::string ofxGgmlStableDiffusionHoloscanBridge::getLastError() const {
#if OFXSD_HAS_HOLOSCAN
	if (impl_->usingHoloscanRuntime && impl_->holoscanState) {
		std::lock_guard<std::mutex> lock(impl_->holoscanState->mutex);
...
const ofxGgmlStableDiffusionHoloscanSettings&
ofxGgmlStableDiffusionHoloscanBridge::getSettings() const {
