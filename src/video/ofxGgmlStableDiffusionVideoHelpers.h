...
inline const char * ofxGgmlStableDiffusionVideoModeName(ofxGgmlStableDiffusionVideoMode mode) {
	switch (mode) {
	case ofxGgmlStableDiffusionVideoMode::Loop: return "Loop";
	case ofxGgmlStableDiffusionVideoMode::PingPong: return "PingPong";
	case ofxGgmlStableDiffusionVideoMode::Boomerang: return "Boomerang";
	case ofxGgmlStableDiffusionVideoMode::Standard:
	default:
		return "Standard";
	}
}

inline float ofxGgmlStableDiffusionVideoDurationSeconds(std::size_t frameCount, int fps) {
...
inline int ofxGgmlStableDiffusionVideoFrameIndexForTime(std::size_t frameCount, int fps, float seconds) {
	if (frameCount == 0) {
		return -1;
	}
	if (fps <= 0) {
...
inline bool ofxGgmlStableDiffusionTryMultiplyInt64(int64_t a, int64_t b, int64_t& result) {
...
inline bool ofxGgmlStableDiffusionTryAddInt64(int64_t a, int64_t b, int64_t& result) {
	const int64_t maxValue = std::numeric_limits<int64_t>::max();
	const int64_t minValue = std::numeric_limits<int64_t>::min();
	if (b > 0 && a > maxValue - b) {
		return false;
...
inline std::vector<int> ofxGgmlStableDiffusionBuildVideoFrameSequence(
	...
	switch (mode) {
	case ofxGgmlStableDiffusionVideoMode::Loop:
		sequence.push_back(0);
		break;
	case ofxGgmlStableDiffusionVideoMode::PingPong:
		for (int i = sourceFrameCount - 2; i > 0; --i) {
			sequence.push_back(i);
		}
		break;
	case ofxGgmlStableDiffusionVideoMode::Boomerang:
...
inline ofxGgmlStableDiffusionVideoRequest ofxGgmlStableDiffusionCreatePromptInterpolationRequest(
	const std::vector<ofxGgmlStableDiffusionPromptKeyframe>& keyframes,
	int frameCount,
	int width = 576,
	int height = 1024,
	ofxGgmlStableDiffusionInterpolationMode mode = ofxGgmlStableDiffusionInterpolationMode::Smooth) {
...
inline ofxGgmlStableDiffusionVideoRequest ofxGgmlStableDiffusionCreateParameterAnimationRequest(
...
inline ofxGgmlStableDiffusionVideoRequest ofxGgmlStableDiffusionCreateSeedSequenceRequest(
	int64_t startSeed,
	int frameCount,
...
inline std::string ofxGgmlStableDiffusionGetFramePrompt(
	...
	if (request.animationSettings.enablePromptInterpolation &&
		...
		return ofxGgmlStableDiffusionInterpolatePrompts(
			request.animationSettings.promptKeyframes,
			frameNumber,
			request.animationSettings.promptInterpolationMode);
	}

	const std::string keyframedPrompt = ofxGgmlStableDiffusionGetKeyframedString(
		request.animationSettings.parameterKeyframes,
		frameNumber,
		[](const ofxGgmlStableDiffusionKeyframe& keyframe) { return keyframe.prompt; });
	return keyframedPrompt.empty() ? request.prompt : keyframedPrompt;
}

inline std::string ofxGgmlStableDiffusionGetFrameNegativePrompt(
	const ofxGgmlStableDiffusionVideoRequest& request,
	int frameNumber) {
	const std::string keyframedNegativePrompt = ofxGgmlStableDiffusionGetKeyframedString(
		request.animationSettings.parameterKeyframes,
		frameNumber,
		[](const ofxGgmlStableDiffusionKeyframe& keyframe) { return keyframe.negativePrompt; });
	return keyframedNegativePrompt.empty() ? request.negativePrompt : keyframedNegativePrompt;
}

inline float ofxGgmlStableDiffusionGetFrameCfgScale(
	const ofxGgmlStableDiffusionVideoRequest& request,
	int frameNumber) {
	if (request.animationSettings.enableParameterAnimation &&
		!request.animationSettings.parameterKeyframes.empty()) {
		const float value = ofxGgmlStableDiffusionGetInterpolatedParameter(
			request.animationSettings.parameterKeyframes,
			frameNumber,
			request.animationSettings.parameterInterpolationMode,
			[](const ofxGgmlStableDiffusionKeyframe& keyframe) { return keyframe.cfgScale; });
		if (value >= 0.0f) {
			return value;
		}
	}
...
inline float ofxGgmlStableDiffusionGetFrameStrength(
	const ofxGgmlStableDiffusionVideoRequest& request,
	int frameNumber) {
	if (request.animationSettings.enableParameterAnimation &&
		!request.animationSettings.parameterKeyframes.empty()) {
		const float value = ofxGgmlStableDiffusionGetInterpolatedParameter(
			...
			frameNumber,
			request.animationSettings.parameterInterpolationMode,
			[](const ofxGgmlStableDiffusionKeyframe& keyframe) { return keyframe.strength; });
		if (value >= 0.0f) {
			return value;
		}
	}
...
inline int64_t ofxGgmlStableDiffusionGetFrameSeed(
	const ofxGgmlStableDiffusionVideoRequest& request,
	int frameNumber) {
	if (request.animationSettings.enableParameterAnimation &&
		!request.animationSettings.parameterKeyframes.empty()) {
		const int64_t keyframedSeed = ofxGgmlStableDiffusionGetKeyframedSeed(
			request.animationSettings.parameterKeyframes,
			frameNumber);
		if (keyframedSeed >= 0) {
...
#else
		int64_t delta = 0;
		if (!ofxGgmlStableDiffusionTryMultiplyInt64(frameOffset, increment, delta)) {
			return ((frameOffset < 0) != (increment < 0)) ? minValue : maxValue;
		}
		int64_t expandedValue = 0;
		if (!ofxGgmlStableDiffusionTryAddInt64(request.seed, delta, expandedValue)) {
			return delta < 0 ? minValue : maxValue;
		}
		return expandedValue;
#endif
