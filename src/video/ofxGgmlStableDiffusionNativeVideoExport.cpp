...
bool saveAvi(const std::string& path, const ofxGgmlStableDiffusionVideoClip& clip, int quality) {
	...
	if (!firstFrame.isAllocated()) {
		ofLogWarning("ofxGgmlStableDiffusion") << "Video export skipped because the first frame is not allocated.";
		return false;
	}
	...
	for (std::size_t i = 0; i < clip.frames.size(); ++i) {
		...
		if (static_cast<uint32_t>(frame.width()) != width || static_cast<uint32_t>(frame.height()) != height) {
			ofLogWarning("ofxGgmlStableDiffusion") << "Video export skipped because frame sizes do not match.";
			return false;
		}
	}
	...
	if (!file) {
		ofLogError("ofxGgmlStableDiffusion") << "Could not open video file for writing: " << resolvedPath;
		return false;
	}
	...
	for (std::size_t i = 0; i < clip.frames.size(); ++i) {
		...
		if (!ofSaveImage(clip.frames[i].pixels, jpegBuffer, OF_IMAGE_FORMAT_JPEG, qualityLevel)) {
			ofLogError("ofxGgmlStableDiffusion") << "Failed to encode frame " << i << " as JPEG for AVI export.";
			return false;
		}
