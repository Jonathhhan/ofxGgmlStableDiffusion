...
This document summarizes all improvements made to the ofxGgmlStableDiffusion addon during the comprehensive review and improvement session in April 2026.
...
**Changes:**
- `src/ofxGgmlStableDiffusion.cpp:919-930` - Fixed `loadImage()` to properly copy pixel data through `OwnedImage::assign()` instead of casting away constness
- `src/ofxGgmlStableDiffusion.cpp:2133-2141` - Added documentation noting `buildOutputImageViews()` creates read-only views
- `src/core/ofxGgmlStableDiffusionNativeAdapter.h:345-347` - Documented that const_cast is required for C API compatibility
...
**Changes:**
- `src/ofxGgmlStableDiffusionThread.h:23-54` - Enhanced `OwnedImage::assign()` with overflow checking
...
**Changes:**
- `src/ofxGgmlStableDiffusion.h:352-354` - Changed `isTextToImage` and `isImageToVideo` to `std::atomic<bool>`
- `src/ofxGgmlStableDiffusion.cpp:890-891, 1571-1572` - Updated assignments to use `.store()` with `memory_order_relaxed`
- `src/ofxGgmlStableDiffusionThread.cpp:380, 394` - Removed redundant flag checks (task enum is authoritative)
...
**Changes:**
- `src/ofxGgmlStableDiffusion.cpp:1681-1779` - Wrapped `applyImageRequest()` with try-catch blocks
- `src/ofxGgmlStableDiffusion.cpp:1781-1840` - Wrapped `applyVideoRequest()` with try-catch blocks
...
**Changes:**
- `src/ofxGgmlStableDiffusion.h:31-238` - Added detailed Doxygen comments to 40+ public methods
