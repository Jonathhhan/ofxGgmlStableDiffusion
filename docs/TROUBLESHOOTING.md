...
This guide helps you diagnose and resolve common issues with ofxGgmlStableDiffusion. Issues are organized by symptom with step-by-step solutions.
...
if (caps.modelFamily == ofxGgmlStableDiffusionModelFamily::Unknown) {
...
std::vector<ofxGgmlStableDiffusionImageFrame> savedImages;
...
if (sd.wasCancelled()) {
    ...
    if (error.code == ofxGgmlStableDiffusionErrorCode::Cancelled) {
...
# Set correct rpath
install_name_tool -add_rpath @loader_path/../../../addons/ofxGgmlStableDiffusion/libs/stable-diffusion/lib/osx bin/myApp.app/Contents/MacOS/myApp
```
...
auto lora = ofxGgmlStableDiffusionLora();
