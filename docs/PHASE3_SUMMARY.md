...
**Solution:**
Created `src/core/ofxGgmlStableDiffusionLimits.h` with:
...
- Use centralized constants from `ofxGgmlStableDiffusionLimits`
...
```cpp
if (!ofxGgmlStableDiffusionLimits::isValidDimension(userWidth)) {
...
```cpp
#include "ofxGgmlStableDiffusion.h"
// ofxGgmlStableDiffusionLimits namespace is now available
```
...
1. **src/core/ofxGgmlStableDiffusionLimits.h** (new) - 193 lines
   - All validation constants
   - Helper validation functions
   - Comprehensive documentation

2. **src/ofxGgmlStableDiffusion.cpp** - Modified validation functions
   - Added include for limits header
   - Updated 10 validation functions
   - Dynamic error messages

3. **src/ofxGgmlStableDiffusion.h** - Added include
...
1da5c7d - Expose validation limits as queryable constants
  - Add ofxGgmlStableDiffusionLimits.h with all validation constants
