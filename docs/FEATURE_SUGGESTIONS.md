# Feature Suggestions for ofxGgmlStableDiffusion
...
Implemented via `ofxGgmlStableDiffusionModelManager` class with:
...
Implemented via `ofxGgmlStableDiffusionQueue` class with:
...
**API Added**:
```cpp
ofxGgmlStableDiffusionQueue queue;
int requestId = queue.addImageRequest(request, ofxGgmlStableDiffusionPriority::High, "batch1");
queue.setCompletionCallback(requestId, [](const ofxGgmlStableDiffusionResult& result) {
    // Handle completion
});
...
Implemented via `ofxGgmlStableDiffusionProgressTracker` class with:
...
**API Added**:
```cpp
ofxGgmlStableDiffusionProgressTracker tracker;
tracker.reset(totalSteps, totalBatches);
tracker.update(currentStep, currentBatch, elapsedSeconds);
tracker.setPhase(ofxGgmlStableDiffusionPhase::Diffusing);
...
Implemented via `ofxGgmlStableDiffusionControlNet` structure and vector-based API:
...
**API Added**:
```cpp
struct ofxGgmlStableDiffusionControlNet {
...
// ImageRequest includes: std::vector<ofxGgmlStableDiffusionControlNet> controlNets;
```
...
**Already Available**:
```cpp
void setLoras(const std::vector<ofxGgmlStableDiffusionLora>& loras);
std::vector<ofxGgmlStableDiffusionLora> getLoras() const;
// ofxGgmlStableDiffusionLora includes name, path, and multiplier (weight) fields
```
...
ofxGgmlStableDiffusionRealtimeSettings rtSettings;
rtSettings.mode = ofxGgmlStableDiffusionRealtimeMode::Streaming;
...
context.quantizationLevel = ofxGgmlStableDiffusionQuantization::Q4_0;
...
context.targetQuantization = ofxGgmlStableDiffusionQuantization::Q5_0;
sd.configureContext(context);
...
auto levels = ofxGgmlStableDiffusionQuantizationHelpers::getAvailableLevels();
...
```cpp
// Use prompt template
auto promptHelper = ofxGgmlStableDiffusionPromptHelpers::getInstance();
...
// Parameter sweep
ofxGgmlStableDiffusionSweepSettings sweepSettings;
sweepSettings.baseRequest = baseRequest;
sweepSettings.parameter = ofxGgmlStableDiffusionParameter::Strength;
sweepSettings.rangeMin = 0.1f;
sweepSettings.rangeMax = 1.0f;
sweepSettings.steps = 10;
sweepSettings.stepMode = ofxGgmlStableDiffusionStepMode::Linear;
...
// A/B comparison
ofxGgmlStableDiffusionImageRequest requestA = baseRequest;
requestA.samplerMethod = EULER_A_SAMPLE_METHOD;

ofxGgmlStableDiffusionImageRequest requestB = baseRequest;
...
std::vector<ofxGgmlStableDiffusionImageRequest> requests;
...
// Lookup by name
auto method = ofxGgmlStableDiffusionSamplingHelpers::getSamplerByName("DPM++ 2M");
auto sched = ofxGgmlStableDiffusionSamplingHelpers::getSchedulerByName("Karras");

// Use presets
auto preset = ofxGgmlStableDiffusionSamplingPreset::Quality();  // DPM++ 2M + Karras, 30 steps
auto preset = ofxGgmlStableDiffusionSamplingPreset::Fast();     // Euler A + Discrete, 15 steps
auto preset = ofxGgmlStableDiffusionSamplingPreset::LCM();      // LCM + LCM scheduler, 4 steps

// Validate and recommend steps
bool valid = ofxGgmlStableDiffusionSamplingHelpers::isValidStepCount(method, 20);
int steps = ofxGgmlStableDiffusionSamplingHelpers::getRecommendedSteps(method, 0.8f);  // quality level 0-1
```
...
ofxGgmlStableDiffusionEmbeddingManager embManager;
...
ofxGgmlStableDiffusionPerformanceProfiler profiler;
...
ofxGgmlStableDiffusionOutpaintingHelper outpaintHelper;
auto outpaintSettings = outpaintHelper.createOutpaintSettings(
    sourceImage,
    ofxGgmlStableDiffusionOutpaintDirection::Right,
...
// Mask preprocessing
ofxGgmlStableDiffusionMaskProcessor maskProcessor;
sd_image_t mask = /* ... */;
...
mask = maskProcessor.createGradientMask(width, height,
    ofxGgmlStableDiffusionMaskGradient::LinearHorizontal);
