...
- Video clip metadata export helpers on both `ofxGgmlStableDiffusionVideoClip` and the wrapper API.
...
- Optional `ofxGgmlStableDiffusionHoloscanBridge` scaffolding for live `frame -> conditioning -> diffusion -> preview` workflows, designed as an addon-side bridge instead of a hard Holoscan dependency.
...
- `ofxGgmlStableDiffusionVideoWorkflowHelpers.h` with reusable video-generation presets, prompt-workflow validation, and richer render-manifest export helpers for request/result based pipelines.
...
- **Multi-ControlNet Support** (Feature #5): Multiple ControlNet models simultaneously
  - `ofxGgmlStableDiffusionControlNet` structure with condition image, strength, and type fields
  ...
  - `controlNets` vector in `ofxGgmlStableDiffusionImageRequest`
...
- **Complete Inpainting Implementation** (Feature #8): Selective image editing with full validation
  ...
  - Complete inpainting mode support with `ofxGgmlStableDiffusionImageMode::Inpainting`
...
- **Performance Profiling System** (Feature #16): Comprehensive performance tracking and analysis
  - `ofxGgmlStableDiffusionPerformanceProfiler` class with timing and memory tracking
...
- **Advanced Sampling Options** (Feature #13): Comprehensive sampler and scheduler configuration
  - `ofxGgmlStableDiffusionSamplingHelpers` with metadata for 15+ samplers and 11+ schedulers
...
- **Video Animation and Interpolation** (Feature #18): Advanced video generation with keyframe support
  ...
  - `ofxGgmlStableDiffusionPromptKeyframe` structure for prompt-based animations
  - `ofxGgmlStableDiffusionKeyframe` structure for parameter animations
  - `ofxGgmlStableDiffusionVideoAnimationSettings` for animation configuration
...
- **Seed Management** (Feature #7): Track generation seeds and reproducibility
  ...
  - `actualSeedUsed` field added to `ofxGgmlStableDiffusionResult` to capture the real seed (including auto-generated ones)

- **Inpainting Mode** (Feature #8 - Partial): Selective image editing with mask support
  - Added `Inpainting` to `ofxGgmlStableDiffusionImageMode` enum
  - Added `maskImage` field to `ofxGgmlStableDiffusionImageRequest` for mask input (white=inpaint, black=keep)
  - Added `maskBlur` parameter for mask edge softening (default: 4.0f)
  - Inpainting task enum added for proper mode handling

- **Model Management System** (Feature #2): Already implemented via `ofxGgmlStableDiffusionModelManager`
...
- **Generation Queue System** (Feature #3): Already implemented via `ofxGgmlStableDiffusionQueue`
...
- **Advanced Progress Reporting** (Feature #4): Already implemented via `ofxGgmlStableDiffusionProgressTracker`
...
- Owned image/video result handling with `ofxGgmlStableDiffusionResult` and `ofxGgmlStableDiffusionVideoClip`
